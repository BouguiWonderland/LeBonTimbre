...
<body>
  ...
  <script type="module">
    ...
    import {IronMeta} from '../iron-meta.js';

    Polymer({
      is: 'meta-test',
      ready: function() {
        this.textContent = new IronMeta({key: 'info'}).value;
      },
    });
    ...
    Polymer({
      ...
      ready: function() {
        var resultList = new IronMeta({type: 'type1'}).list;
    ...
    Polymer({
      ...
      ready: function() {
        var resultList = new IronMeta({type: 'type2'}).list;
        this.textContent = JSON.stringify(resultList);
      },
    });
  </script>
